...
class ResizeShuffleBoxes(object):
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None):
        ...
        labels = np.concatenate((labels, false_object_pre), axis=1)
...
class ToTensor(object):
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None):
        ...
        boxes_pre[1] = torch.from_numpy(boxes_pre[1].astype(np.float32))

        boxes_next[0] = torch.from_numpy(boxes_next[0].astype(np.float32))
        boxes_next[1] = torch.from_numpy(boxes_next[1].astype(np.float32))

        labels = torch.from_numpy(labels.astype(np.float32)).unsqueeze(0)
...
class ConvertFromInts(object):
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None):
        return [obj.astype(np.float32) for obj in (img_pre, img_next, boxes_pre, boxes_next, labels)]
...
class RandomSampleCrop(object):
    ...
    def crop(self, image, boxes, labels, mode, min_iou, max_iou, w, h, left, top, isPre=True):
        ...
        # pading the mask
        mask = np.pad(mask, (0, Config.max_object - len(mask)), 'constant')
        mask = (mask == False)
