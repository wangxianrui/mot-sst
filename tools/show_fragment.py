...
def write_video(res_raw, img_dir, colorList, vwriter):
    for t in range(min(res_raw[:, 0]), max(res_raw[:, 0]) + 1):
        img_name = os.path.join(img_dir, '{:06}.jpg'.format(t))
...
def main(args):
    ...
    for txt_name in txt_list:
        ...
        temp_img = cv2.imdecode(np.fromfile(os.path.join(img_dir, '000001.jpg'), dtype=np.uint8), -1)
        h, w, _ = temp_img.shape
        with open(txt_file, 'r') as file:
            data = file.read().splitlines()
            index = 0
            while index < len(data):
                start, end = re.findall(r'\d+', data[index])[slice(0, 3, 2)]
                avi_file = os.path.join(Config.result_dir, args.type, 'avi',
                                        txt_name[:-4] + '_' + start.zfill(6) + '_' + end.zfill(6) + '.avi')
                print(avi_file)
                vwriter = cv2.VideoWriter(avi_file, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 25, (w, h))
                index += 1
                res_raw = []
                while index < len(data) and 'fragment:' not in data[index]:
                    res_raw.append([float(n) for n in data[index].split(',')])
                    index += 1
                res_raw = np.asarray(res_raw).astype(np.int)
                write_video(res_raw, img_dir, colorList, vwriter)
